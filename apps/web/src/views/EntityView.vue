<template>
  <v-card>
    <v-card-title class="card-title-row flex-wrap">
      <span class="text-h6">{{ t(titleKey) }}</span>
      <div class="d-flex flex-wrap align-center ga-2" style="margin-left: auto">
        ...
        <v-select
          ...
          item-title="label"
          item-value="value"
          :label="t('entity.sortDirection')"
          density="compact"
          style="min-width: 130px"
        />
        <v-text-field
          v-model="search"
          :label="t('entity.searchLabel')"
          density="compact"
          style="min-width: 240px"
          @keyup.enter="onSearch"
        />
        <v-btn color="primary" variant="tonal" :loading="busy" :disabled="busy" @click="onSearch">{{ t("common.query") }}</v-btn>
      </div>
    </v-card-title>
    ...
    <v-card-text>
      ...
      <v-alert v-if="selectedCount > 0" type="info" class="mb-4">{{ t("entity.selectedCount", { count: selectedCount }) }}</v-alert>
      ...
      <div class="d-flex flex-wrap justify-space-between align-center ga-3 mt-4">
        <p class="text-body-2 text-medium-emphasis mb-0">{{ t("entity.pageSummary", { total, page, pageSize }) }}</p>

        <div class="d-flex flex-wrap align-center ga-2">
          <v-select v-model="pageSize" :items="pageSizeOptions" :label="t('common.pageSize')" style="max-width: 120px" />
          <v-btn variant="outlined" :disabled="busy || page <= 1" @click="prevPage">{{ t("common.prevPage") }}</v-btn>
          <v-btn variant="outlined" :disabled="busy || page * pageSize >= total" @click="nextPage">{{ t("common.nextPage") }}</v-btn>
        </div>
      </div>
      ...
      <div class="d-flex flex-wrap justify-space-between align-center ga-2 mt-4">
        <div class="d-flex flex-wrap align-center ga-2">
          <v-btn color="error" :disabled="selectedCount === 0 || busy" @click="onPreview">
            {{ t(actionLabelKey) }} ({{ selectedCount }})
          </v-btn>
          <v-btn
            v-if="supportsQuickCleanup"
            color="error"
            variant="outlined"
            :disabled="busy"
            @click="onPreviewAll"
          >
            {{ t("entity.quickCleanupAll") }}
          </v-btn>
        </div>
      ...
      <v-alert v-if="supportsQuickCleanup" type="info" class="mt-3">
        {{ t("entity.quickCleanupHint") }}
      </v-alert>
  ...
  <ConfirmModal
    :open="confirmOpen"
    :title="t('entity.dangerConfirmTitle')"
    :summary="t('entity.dangerConfirmSummary', { action: t(actionLabelKey), count: pendingCount })"
...
import { createPreview, executeBatch, fetchEntities, previewNonFriendChats } from "../services/api";
...
const supportsQuickCleanup = computed(
  () => props.type === "non_friend_chat" && props.action === "CLEANUP_NON_FRIEND_CHATS"
);
...
const sortByOptions = computed(() => [
  { label: t("entity.sortFieldTitle"), value: "title" },
  { label: t("entity.sortFieldLastUsed"), value: "last_used_at" }
]);

const sortOrderOptions = computed(() => [
  { label: t("entity.sortOrderDesc"), value: "desc" },
  { label: t("entity.sortOrderAsc"), value: "asc" }
]);
...
const actionLabelKey = computed(() => props.actionLabelKey);
...
const onPreviewAll = async () => {
  if (!supportsQuickCleanup.value) {
    return;
  }
  ...
  try {
    const preview = await previewNonFriendChats();
    pendingPreview.value = preview;
    previewWarnings.value = preview.warnings;
    confirmOpen.value = true;
  } catch (e) {
